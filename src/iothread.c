...
void assignClientToIOThread(client *c) {
    if (c->tid != IOTHREAD_MAIN_THREAD_ID) {
        serverLog(LL_WARNING, "c->tid(%d)", c->tid);
    }
    serverAssert(c->tid == IOTHREAD_MAIN_THREAD_ID);
    ...
    if (server.io_threads_scale_status == IO_THREAD_SCALE_STATUS_UP) {
        if ((IOThreads[min_id].io_thread_scale_status == IO_THREAD_SCALE_STATUS_NONE ||
            (IOThreads[1].io_thread_scale_status == IO_THREAD_SCALE_STATUS_UP) )) {
            /*
            ...
            * - IO thread 1 is scaling (UP): means all IO threads are new (cold start),
            *   so finish immediately.
            */
            ioThreadsScaleUpEnd();
        }
    }
...
void ioThreadsScaleUpStart(void) {
    ...
    serverLog(LL_NOTICE, "IO threads scale-up start %d => %d", old_io_threads_num ,server.io_threads_num);
    server.io_threads_scale_status = IO_THREAD_SCALE_STATUS_UP;
    /* When the number of clients is less than the number of threads, load balancing is not needed. */
    if (all_clients_num < server.io_threads_num) {
        serverLog(LL_NOTICE, "IO threads scale-up client num(%d)< thread num(%d)", all_clients_num ,server.io_threads_num);
        ioThreadsScaleUpEnd();
    } 
}
