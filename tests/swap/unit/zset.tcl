    ...
    proc swap_zset {big} {
        ...
        proc stressers {encoding} {
            ...
            test "ZSCORE - $encoding" {
                ...
                for {set i 0} {$i < $elements} {incr i} {
                    floats_equal [lindex $aux $i] [r zscore zscoretest $i]
                }
            }
            ...
            test "ZMSCORE - $encoding" {
                ...
                assert_encoding $encoding zscoretest
                for {set i 0} {$i < $elements} {incr i} {
                    floats_equal [lindex $aux $i] [r zmscore zscoretest $i]
                }
            }
...
start_server {tags {"zsetConvert bug listpack"}} {
    set listpack_client [srv 0 client]
    set listpack_host [srv 0 host]
    set listpack_port [srv 0 port]
    $listpack_client config set zset-max-listpack-entries 256
    $listpack_client config set zset-max-listpack-value 64
    $listpack_client config set swap-debug-evict-keys 0
    start_server {tags {"zsetConvert bug skiplist"}} {
        ...
        $skiplist_client config set zset-max-listpack-entries 0
        $skiplist_client config set zset-max-listpack-value 0
        ...
        proc write_data_and_slaveof_master {elements master_encoding master_client slave_client master_host master_port} {
            ...
            # assert_match "* encoding:$master_encoding *" $dbg
            for {set i 0} {$i < $elements} {incr i} {
                floats_equal [lindex $aux $i] [$slave_client zscore zscoretest $i]
            }
            $slave_client slaveof no one
        }
        test "swap master(listpack)-slave(skiplist)" {
            write_data_and_slaveof_master 128 listpack $listpack_client $skiplist_client $listpack_host $listpack_port
        }
        test "swap master(skiplist)-slave(listpack)" {
            write_data_and_slaveof_master 128 skiplist $skiplist_client $listpack_client $skiplist_host $skiplist_port
        }
    }
