start_server {overrides {save ""} tags {"swap" "rdb"}} {
    test {keyspace and rocksdb obj encoding not binded} {
        ...
        assert_equal [r object encoding hashbig] listpack
...
start_server {tags {"master-slave rdb-size (repl-rdb-channel no)"}} {
    start_server {} {
...
start_server {tags {"master-slave rdb-size (repl-rdb-channel yes)"}} {
    start_server {} {
        set master [srv -1 client]
        set master_host [srv -1 host]
        set master_port [srv -1 port]
        ...
        $master config set repl-diskless-sync no
        $master config set swap-repl-rordb-sync yes
        $slave config set repl-rdb-channel yes
        check_master_slave_rdb_size $master $slave $master_host $master_port $rordb
        ...
        $master config set swap-repl-rordb-sync yes
        $slave config set repl-rdb-channel yes
        check_master_slave_rdb_size $master $slave $master_host $master_port 0
